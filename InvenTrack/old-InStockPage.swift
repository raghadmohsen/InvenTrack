...
class Inventory: ObservableObject {
    @Published var products: [Product] = [
        Product(name: "MacBook Air", quantity: 20, imageName: "mac"),
        Product(name: "Airpods", quantity: 20, imageName: "air")
    ]
}

struct InStockPage: View {
    @ObservedObject var inventory = Inventory() // استخدام ObservableObject
                                ...
                                .contextMenu { // قائمة السياق
                                    ...
                                    Button(role: .destructive, action: {
                                        if let index = inventory.products.firstIndex(where: { $0.id == product.id }) {
                                            ...
                                            // تحديث الكمية أو تنفيذ أي إجراءات أخرى
                                        }
                                    }) {
